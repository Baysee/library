...
lent=2^22;
time_window=200e-9;%second. Actual time array=+-time_window/2
...
m=10;p=1;%% keep p=1 for now (need to update next line)
...
% 
% 
% % og_sig_f=generatePeriodicGauss(f,nuR,df*5,1); %og_sig_f=circshift(og_sig_f,round(nuR/2/df));
 og_sig=nrmd_ifft(og_sig_f,Fs,scale);

% trunc=SuperGauss(0,time_window/4,5,t,0);
% og_sig=generatePeriodicGauss(t,tR,dt*2,1);%.*trunc; %og_sig_f=circshift(og_sig_f,round(nuR/2/df));
% og_sig_f=nrmd_fft(og_sig,dt,scale);


% % 
%    og_sig=ones(lent,1)';%genDiscPhase(lent,tq,2,dt,[0 1 ],1);ones(1,lent);
%   og_sig=superGauss(0,2e-9,1,t,0);%genDiscPhase(lent,tq,2,dt,[0 1 ],1);ones(1,lent);
% % %  og_sig=circshift(og_sig,-50);
% % % 
%  og_sig_f=nrmd_ifft(og_sig,dt,scale);
% % 
...
% % % % 
% disp=-(710e3)*17e-6;%-10.08;%-(120e3)*17.3e-6-1.3;%-9.956;%-10.08;%120e3*19.46e-6;%10.08;%(590e3)*17.3e-6;%19.64e-6 gives optimal value for 120km;
...
% % 
[phaseTemporalRaw]=genDiscPhase(lent,ts,m,dt,GV,1);
 phaseTemporal=filtSG(phaseTemporalRaw,round(50e9/df/2),1,1)*max(phaseTemporalRaw);
% % phaseTemporal=circshift(phaseTemporal,-262);
...
% % GV=circshift(GV,10);
% [phaseTemporal]=genDiscPhase(lent,ts,m,dt,GV,1); 
% % GV=round(GV/(2*pi)*2^8)/2^8*2*pi;; 
% % phaseTemporal=circshift(phaseTemporal,round(ts/4));
% 
...
ylabel('Intensity')
yyaxis right
plot(tps,(abs(temporalRaw).^2)/nrmFac,'DisplayName','Temporal S-TAI (no OFT)')
hold on
