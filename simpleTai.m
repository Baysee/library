...
m=33;p=1;%% keep p=1 for now (need to update next line)
...
% tq=m*ts;
 ...
 phi2=1.2814e-20% equiv to 587.192 km

%  phi2=p*m*(tq/m)^2/(2*pi)
ts=sqrt(2*pi*phi2/m);
tq=m*ts;
% tq=m*(50.4e-12);%1e-9;%1/1e9;
% ts=tq/m;
...
% % % % %  discSpec=(p/m*pi*(0:m-1).^2);
%  phi2= 5.1119e-21;
%  sqrt(5.1119e-21*(2*pi)/(p*m))*m
%  disp=-10.00;%-(120e3)*17.3e-6-1.3;%-9.956;%-10.08;%120e3*19.46e-6;%10.08;%(590e3)*17.3e-6;%19.64e-6 gives optimal value for 120km;
% lambda=1550e-9;%m
% speedoflight=299792458;%m/s
% phi2= -lambda^2*disp/(2*pi*speedoflight);
%  phi2=p*m*(tq/m)^2/(2*pi)
 phi2=m*ts^2/(2*pi);%1.2814e-20% equiv to 587.192 km
...
% % GV=wrapTo2Pi(p/m*pi*(0:m-1).^2); 
 GV=(-pi*(0:m-1).^2*(m-1)/m); %wrapTo2Pi(s/m*pi*(0:m-1).^2); 
%  GVsum=zeros(size(GV));
%  for iGV=1:m
%      GVsum=exp(1j*(circshift(GV,iGV)-GV(iGV)))+GVsum;
%  end
%  GVcorr=GV/max(GV)
%  GVmeas=[0 0.8 0.16 1.16 241/290 181/290 157/290 157/290 192/290 246/290 1 0.1 0.75]
%   create_AWGfile(s,m,repelem(GV,1,[3]));
...
% phaseTemporal=real(filtSG_tf(phaseTemporal,t,f,round(15e9/df),5,1));%(sig,t,f,nBW,m,displayPlot)*max(phaseTemporal));
% phaseTemporal=circshift(phaseTemporal,-round(621e-12/dt));
